...
void GLGSRender::init_buffers(rsx::framebuffer_creation_context context, bool skip_reading)
{
	...
	for (int i = 0; i < rsx::limits::color_buffers_count; ++i)
	{
		if (m_surface_info[i].pitch && g_cfg.video.write_color_buffers)
		{
			...
			const rsx::address_range surface_range = m_surface_info[i].get_memory_range();
	...
	if (g_cfg.video.write_color_buffers)
	{
		...
		for (u8 i = 0; i < rsx::limits::color_buffers_count; ++i)
		{
			...
			const rsx::address_range surface_range = m_surface_info[i].get_memory_range(layout.aa_factors[1]);
...
void GLGSRender::read_buffers()
{
	...
	if (g_cfg.video.read_color_buffers)
	{
		...
		auto read_color_buffers = [&](int index, int count)
		{
			...
			for (int i = index; i < index + count; ++i)
			{
				...
				const rsx::address_range range = rsx::address_range::create_start_length(texaddr, pitch * height);
	...
	if (g_cfg.video.read_depth_buffer)
	{
		...
		const rsx::address_range range = rsx::address_range::create_start_length(depth_address, pitch * height);
