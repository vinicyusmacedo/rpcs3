...
namespace rsx
{
	...
	public:
		...
		void protect(utils::protection prot)
		{
			...
			page_info.set_protection(*this, prot);
#endif
		}
...
#ifdef TEXTURE_CACHE_DEBUG
	private:
		class page_info_array {
			struct page_info_elmnt {
				u8 prot = 0;
				u8 no = 0;
				u8 ro = 0;
				//u8 rw = 0;
			...
			static_assert(sizeof(page_info_elmnt) <= 4, "page_info_elmnt must be less than 4-bytes in size");

			// 4GB memory space / 4096 bytes per page = 1048576 pages
			page_info_elmnt _info[1048576];

			static constexpr size_t address_to_index(u32 address)
			{
				return (address / 4096) * sizeof(page_info_elmnt);
			}

			static constexpr u32 index_to_rsx_address(size_t idx)
			{
				return static_cast<u32>((idx / sizeof(page_info_elmnt)) * 4096);
			}

			constexpr page_info_elmnt* rsx_address_to_info_pointer(u32 address)
			{
				return &(_info[address_to_index(address)]);
			}

			constexpr const page_info_elmnt* rsx_address_to_info_pointer(u32 address) const
			{
				return &(_info[address_to_index(address)]);
			}

			constexpr u32 info_pointer_to_address(const page_info_elmnt* ptr) const
			{
				return index_to_rsx_address(static_cast<size_t>(
					(ptr - _info) / sizeof(page_info_elmnt)
				));
			}
		public:
			page_info_array()
			...
			void set_protection(const address_range& range, utils::protection prot)
			{
				...
				for (page_info_elmnt* ptr = rsx_address_to_info_pointer(range.start); ptr <= rsx_address_to_info_pointer(range.end); ptr++)
			...
			void reset_refcount()
			{
				for (page_info_elmnt* ptr = rsx_address_to_info_pointer(0); ptr <= rsx_address_to_info_pointer(0xFF'FF'FF'FF); ptr++)
					ptr->reset_refcount();
			}
			...
			std::pair<u16,u16> get_min_max_sum(const address_range& range) const // returns the {min,max} number of sections in a page within the given range
			{
				AUDIT(range.is_page_range());
				u16 min = UINT16_MAX;
				u16 max = 0;
				for (const page_info_elmnt* ptr = rsx_address_to_info_pointer(range.start); ptr <= rsx_address_to_info_pointer(range.end); ptr++)
				{
					min = std::min(min, ptr->sum());
					max = std::max(max, ptr->sum());
				}
				AUDIT(min <= max);
				return { min , max };
			}

			void check_unprotected(const address_range& range) const
			{
				AUDIT(range.is_page_range());
				for (const page_info_elmnt* ptr = rsx_address_to_info_pointer(range.start); ptr <= rsx_address_to_info_pointer(range.end); ptr++)
				{
					...
					if (prot != utils::protection::rw)
					{
						...
						fmt::throw_exception("Page should be unprotected: address=0x%x, Prot=%d, RO=%d, NA=%d", addr, static_cast<u32>(prot), ptr->ro, ptr->no);
					}
				}
			}
			...
			void verify() const
			{
				constexpr size_t size = sizeof(_info) / sizeof(_info[0]);
				for (size_t idx = 0; idx < size; idx++)
				{
					...
					if (!elmnt.verify())
					{
						...
						fmt::throw_exception("Protection verification failed for addr=0x%x: Prot=%d, RO=%d, NA=%d", addr, static_cast<u32>(prot), elmnt.ro, elmnt.no);
					}
				}
			}
		};
	...
	public:
		static page_info_array page_info;
